    ...
    <div className="flex flex-col items-center justify-center h-100 space-y-8">
      ...
      <div className="w-64 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="icon" onClick={togglePlay} className="rounded-full bg-transparent">
            {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
          </Button>
          <span className="text-sm text-muted-foreground">{formatTime(SESSION_DURATION)}</span>
        </div>
      </div>
    </div>
  )
